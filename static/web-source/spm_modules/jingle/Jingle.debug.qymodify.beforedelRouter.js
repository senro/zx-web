...
(function($){
    ...
    function Refresh(selector,type,callback,forceOpenPullDown,forceOpenPullDownCallback){
        var iscroll, scroller,refreshEl,iconEl,labelEl,topOffset,isPullDown,
            options = {
                ...
                callback : undefined,
                forceOpenPullDown:false,
                forceOpenPullDownCallback:undefined,

                forceOpenPullDown_pullText: "下拉刷新...",
                forceOpenPullDown_releaseText: "松开立即刷新...",
                forceOpenPullDown_refreshText: "刷新中...",
                forceOpenPullDown_refreshTip : false,//下拉时显示的文本，比如：最后更新时间:2013-....
                forceOpenPullDown_onPullIcon : 'arrow-down-2'
            };
        //qiuyun 扩展强制开启下拉
        var forceRefreshEl,forceIconEl,forceLabelEl;
        //装载配置
        if(typeof selector === 'object'){
            ...
            options.forceOpenPullDown = forceOpenPullDown;
            options.forceOpenPullDownCallback = forceOpenPullDownCallback;
            if(type === 'pullUp'){
                $.extend(options,{
                    pullText: "上拉加载更多...",
                    releaseText: "松开开立即加载...",
        ...
        var _init = function(opts){
            ...
            if(isPullDown){
                ...
                //qiuyun add 强制开启下拉
                if(opts.forceOpenPullDown){
                    var forceRefreshTpl = '<div class="refresh-container"><span class="refresh-icon icon '+opts.forceOpenPullDown_onPullIcon
                        +'"></span><span class="refresh-label">'
                        +opts.forceOpenPullDown_pullText+'</span>'
                        +(opts.forceOpenPullDown_refreshTip?'<div class="refresh-tip">'+opts.forceOpenPullDown_refreshTip+'</div>':'')+'</div>';
                    forceRefreshEl = $(forceRefreshTpl).prependTo(scroller);
                    forceIconEl = forceRefreshEl.find('.refresh-icon');
                    forceLabelEl = forceRefreshEl.find('.refresh-label');
                }
            }
        ...
        var _excuteScroll = function(opts){
            return J.Scroll(opts.selector,{
                    ...
                    onScrollMove : function(){
                        ...
                        }else if (this.y>0 && this.y > opts.minPullHeight && !isPullDown && opts.forceOpenPullDown && !forceIconEl.hasClass(opts.onReleaseIcon)) {
                            forceRefreshEl.show();
                            //上拉中的下拉
                            forceIconEl.addClass(opts.onReleaseIcon);
                            forceLabelEl.html(opts.forceOpenPullDown_releaseText);
                            this.minScrollY = 0;
                        } else if (this.y>0 && this.y < opts.minPullHeight && !isPullDown && opts.forceOpenPullDown && forceIconEl.hasClass(opts.onReleaseIcon)) {
                            forceRefreshEl.show();
                            //上拉中的下拉
                            forceIconEl.removeClass(opts.onReleaseIcon);
                            forceLabelEl.html(opts.forceOpenPullDown_pullText);
                            this.minScrollY = -topOffset;
                        }else if (this.y<0 && this.y < (this.maxScrollY - opts.minPullHeight) && !isPullDown && !iconEl.hasClass(opts.onReleaseIcon)) {
                            refreshEl.show();
                            //上拉
                            iconEl.addClass(opts.onReleaseIcon);
                            labelEl.html(opts.releaseText);
                    ...
                    onScrollEnd : function(){
                        if(iconEl.hasClass(opts.onReleaseIcon)){
                            ...
                            setTimeout(function(){//解决在chrome下onRefresh的时候文本无法更改的问题。奇怪的问题！
                                opts.callback.call(_this,refreshEl);
                            },1);

                            forceRefreshEl.hide();
                        }else if(opts.forceOpenPullDown && forceIconEl.hasClass(opts.onReleaseIcon)){
                            //qiuyun add 强制开启下拉
                            forceIconEl.removeClass(opts.onReleaseIcon).removeClass(opts.forceOpenPullDown_onPullIcon).addClass(opts.onRefreshIcon);
                            forceLabelEl.html(opts.forceOpenPullDown_refreshText);
                            var _this = this;
                            setTimeout(function(){//解决在chrome下onRefresh的时候文本无法更改的问题。奇怪的问题！
                                opts.forceOpenPullDownCallback.call(_this,forceRefreshEl);
                            },1);

                            refreshEl.hide();//todo 可能引起问题，待测试
                        }
                    },
                    onRefresh: function () {
                        iconEl.removeClass(opts.onReleaseIcon).removeClass(opts.onPullIcon).addClass(opts.onRefreshIcon);
     ...
     * @param callback 回调函数
     */
    J.Refresh = function(selector,type,callback,forceOpenPullDown,forceOpenPullDownCallback){
        var el,jRefreshId;
        ...
        if(jRefreshId && refreshCache[jRefreshId]){
            ...
            var refresh = new Refresh(selector,type,callback,forceOpenPullDown,forceOpenPullDownCallback);
            return refreshCache[jRefreshId] = {
